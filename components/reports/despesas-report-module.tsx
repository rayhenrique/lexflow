...
interface ExpenseReportRow {
  ...
  expense_classifications: { name: string } | null;
}
...
export function DespesasReportModule() {
  ...
  const clientsQuery = useMemo(
    () => ["report-despesas-clients", selectedWorkspaceId] as const,
    [selectedWorkspaceId],
  );
  const classificationsQuery = useMemo(
    () => ["report-despesas-classifications", selectedWorkspaceId] as const,
    [selectedWorkspaceId],
  );
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      fetchExpenseClassifications(supabase, classificationsQuery[1]),
    ])
  ...
  async function runPreview() {
    ...
    try {
      let query = supabase
        .from("expenses")
        .select("*, clients(name), expense_classifications(name)")
      ...
      setRows((data ?? []) as ExpenseReportRow[]);
      setDidPreview(true);
    } catch (error) {
  ...
  function handleExportExcel() {
    ...
    const exportRows = rows.map((row) => ({
      ...
      Categoria: row.expense_classifications?.name ?? "-",
      Status: row.status,
      Valor: Number(row.amount),
    }));

    exportJsonToCsv(exportRows, "relatorio_despesas_lexflow.csv");
  }
                  ...
                  <Command>
                    ...
                    <CommandList>
                      ...
                      <CommandGroup>
                        ...
                        >
                          <Check
                            className={cn("mr-2 h-4 w-4", filters.classificationId === "all" ? "opacity-100" : "opacity-0")}
                          />
                          Todas
                        </CommandItem>
                        {classifications.map((classification) => (
                          <CommandItem
                            ...
                            onSelect={() => {
                              setFilters((prev) => ({ ...prev, classificationId: classification.id }));
                              setClassificationOpen(false);
                            }}
                          >
                            <Check
                              className={cn(
                                "mr-2 h-4 w-4",
                                filters.classificationId === classification.id ? "opacity-100" : "opacity-0",
                              )}
                            />
      ...
      {didPreview ? (
        <div className="space-y-3 print:space-y-2">
          ...
          <p className="text-sm text-zinc-500">
            Total: <span className="font-semibold text-rose-600">{currencyFormatter.format(totalAmount)}</span>
          </p>
                  ...
                  {loadingPreview ? (
                    ...
                    paginatedRows.map((row) => (
                      <tr key={row.id} className="border-b border-zinc-200 last:border-b-0">
                        ...
                        <td className="px-3 py-2 text-zinc-700">{row.expense_classifications?.name ?? "-"}</td>
                        <td className="px-3 py-2 text-zinc-700">
